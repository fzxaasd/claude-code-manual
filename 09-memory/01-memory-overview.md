...
Claude Code 使用 **六层内存架构** 来管理不同层级的指令和上下文：
...
基于 `MEMORY_TYPE_VALUES`（定义于 `src/utils/memory/types.ts`，注意：非 `src/memdir/memoryTypes.ts`，后者定义的是内存内容分类类型 `user/feedback/project/reference`）:
