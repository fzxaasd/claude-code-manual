...
### 13. Hidden/Undocumented Options
...
|------|------|------|
| `--agent-id <id>` | string | Teammate agent ID (generated by leader) |
| `--agent-name <name>` | string | Teammate display name |
| `--agent-color <color>` | string | Teammate UI color |
| `--agent-type <type>` | string | Custom agent type for this teammate |
| `--agent-teams` | boolean | Force Claude to use multi-agent mode |
| `--team-name <name>` | string | Swarm coordination team name |
| `--teammate-mode <mode>` | string | Teammate spawn mode: "tmux", "in-process", or "auto" |
| `--plan-mode-required` | boolean | Require plan mode before implementation |
| `--parent-session-id <id>` | string | Parent session ID for analysis correlation |

#### Feature-Gated Options

| Parameter | Type | Description |
|------|------|------|
| `--advisor <model>` | string | Enable server-side advisor tool |
| `--enable-auto-mode` | boolean | Opt-in to auto mode |
| `--proactive` | boolean | Start proactive autonomous mode |
| `--brief` | boolean | Enable SendUserMessage tool for inter-agent communication |
| `--tasks [id]` | string | Task mode: monitor tasks and auto-process. Optional id as tasklist and agent ID |
| `--channels <servers...>` | string[] | MCP servers whose channel notifications should register this session |
| `--dangerously-load-development-channels <servers...>` | string[] | Load off-whitelist channel servers (dev only) |

#### SDK & Remote Options

| Parameter | Type | Description |
|------|------|------|
| `--sdk-url <url>` | string | Use remote WebSocket endpoint for SDK I/O streaming (with -p and stream-json only) |
| `--assistant` | boolean | Force assistant mode (Agent SDK daemon use) |
| `--teleport [session]` | string | Resume teleport session, optional session ID |
| `--messaging-socket-path <path>` | string | Unix domain socket path for UDS message server |
| `--hard-fail` | boolean | Crash on logError calls instead of silent logging |

#### Deep Link Options

| Parameter | Type | Description |
|------|------|------|
| `--deep-link-repo <slug>` | string | Repository slug parsed from deep link ?repo= param |
| `--deep-link-last-fetch <ms>` | number | FETCH_HEAD mtime (milliseconds) |
...
| `--dangerously-skip-permissions-with-classifiers` | Deprecated `--permission-mode auto` alias |
| `--afk` | Deprecated `--permission-mode auto` alias |

#### Cowork Options (Hidden)

| Parameter | Type | Description |
|------|------|------|
| `--cowork` | boolean | Use cowork_plugins directory (hidden) |
| `--enable-auth-status` | boolean | Enable auth status messages in SDK mode |
| `--permission-prompt-tool <tool>` | string | MCP tool for permission prompts |
